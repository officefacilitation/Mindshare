...
export const DetailPanel: React.FC<DetailPanelProps> = ({
      ...
      <aside className="hidden xl:block w-72 h-[calc(100vh-57px)] sticky top-[57px] bg-canvas hairline-l p-6 text-center select-none shrink-0">
        <div className="h-full flex flex-col items-center justify-center text-ink-muted">
          ...
          <p className="text-xs text-ink-muted leading-relaxed max-w-[200px]">
            Click any note card in the center feed to inspect full content, AI summaries, tags, and contacts.
          </p>
        </div>
      </aside>
    );
  ...
  const handleRemoveTag = async (tagName: string) => {
    ...
    const res = await onUpdateNote(note.id, updatedContent || note.content);
  ...
  return (
    <>
      <aside className="w-full xl:w-72 h-full xl:h-[calc(100vh-57px)] xl:sticky xl:top-[57px] bg-surface xl:bg-canvas hairline-l flex flex-col p-5 overflow-y-auto shrink-0">
        {/* Header */}
        ...
        <div className="flex-1 space-y-5">
          ...
          <div>
            ...
            {isEditing ? (
              ...
              <div className="p-3.5 rounded-lg bg-surface hairline-border text-sm text-ink leading-relaxed whitespace-pre-wrap font-sans">
                {note.content}
              </div>
            )}
          </div>
